...
def run_checked(sandbox: Sandbox, command: str, *, timeout: float = 60) -> str:
    try:
        ...
        raise RuntimeError(f"Command failed before completion: {command}\n{type(exc).__name__}: {exc}") from exc
...
def main() -> None:
    ...
    required_env = ["CUBE_TEMPLATE_ID", "E2B_API_URL", "E2B_API_KEY"]
    ...
    if not snapshot_id:
        raise RuntimeError("Snapshot was not created")
